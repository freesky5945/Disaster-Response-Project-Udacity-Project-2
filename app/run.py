...
def index():
    ...
    # Top ten categories
    top_category_direct_count = df[df['genre'] == 'direct'].iloc[:,4:].sum().sort_values(ascending=False)[0:10]
    ...
    top_category_social_count = df[df['genre'] == 'social'].iloc[:,4:].sum().sort_values(ascending=False)[0:10]
